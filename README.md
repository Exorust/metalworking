...
compiler — at exact file/line coordinates, in an order that builds from "what is this
hardware" to "why did my hand-written kernel lose to stock MLX."
...
| [2 — The GEMM ladder](track/stage-2-gemm.md) | Naive → tiled → double-buffered → async-DMA matrix multiply | You can say what double buffering costs and buys, and beat MPS at one size |
...
These recur independently across every project in this track — luminal, MLX, llama.cpp,
and every Reddit war story converge on them:
...
2. **Your custom kernel will probably lose to stock MLX steel / ggml kernels.** Multiple
   independent reports. The real wins are in dispatch/sync overhead, unlocking existing
   fast paths, and deleting work.
3. **F16 everywhere; avoid FP32 atomics** (emulated, slow). Register pressure is the real
   budget: ~208 KB per core.
...
other people's work, fetched from its original home under its own license — nothing there
is vendored or redistributed here. Full attribution is in [SOURCES.md](SOURCES.md) and in
each stage.
