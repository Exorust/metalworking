...
Needs `code/Metal-Puzzles` and `code/tinygrad-notes`. Time: a weekend, hands on keyboard.
...
- `code/Metal-Puzzles/metal_puzzles.ipynb` is the intended interactive form (or
...
- `code/Metal-Puzzles/utils.py`: read this once you're a few puzzles in. It's the
  MLX-backed harness that compiles your MSL string, runs it, and diffs against the spec.
  It's also the minimal "compile and dispatch an MSL kernel from Python via
  `mx.fast.metal_kernel`" pattern, and you'll reuse it for your own experiments for the
  rest of the track.

## Read alongside

`code/tinygrad-notes/20240921_metal.md`, "Abstraction in Apple's Metal Framework".
It strips Metal down to the raw C / Objective-C calls (device, library, pipeline, command
...
- All 14 puzzles pass.
- You can draw the hierarchy (grid, threadgroup, simdgroup of 32 threads, thread) and
  say which level shares which memory.
