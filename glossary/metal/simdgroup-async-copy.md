...
**`simdgroup_async_copy` was Apple's undocumented DMA intrinsic — hardware-driven
asynchronous tile copies from device to threadgroup memory — and it no longer
compiles. It's in this glossary because its ghost explains the shape of current
kernels.**
...
— the DMA hardware does the moving, so parallelizing the ask bought nothing.
...
What fills the gap on current toolchains: [cooperative loads](../techniques/cooperative-load.md)
...
scheduler interleaves. That substitution — DMA overlap → ILP overlap — is the
single clearest example of why current Apple kernels look different from their
