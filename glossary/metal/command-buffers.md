...
community-converged pattern for inference: **one command buffer per forward pass**
— encode every layer's dispatches, commit once, wait once.
...
granularity, which is one more reason to make command buffers meaningful units of
work.
