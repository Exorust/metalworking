...
**MPS is Apple's library of pre-built GPU kernels — matmul, convolution, image ops
— the closest thing Metal has to cuBLAS/cuDNN, and the baseline your custom kernel
has to beat.**
...
For the kernel reader, MPS's main role is **the benchmark floor**: `MPSMatrixMultiplication`
is a three-object setup (matrix descriptors + the op) and gives honest GPU-time
numbers via [command-buffer timestamps](profiling.md) — the
...
ship MPS — that's not defeat, that's the [three questions](../war-stories/three-questions.md)
working as intended.
