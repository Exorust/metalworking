...
slower**, not 10% slower, because the accumulators spilled. When an Apple kernel is
...
- **[`max_total_threads_per_threadgroup`](../metal/function-constants.md)**: an
  attribute that promises the compiler your threadgroup size, so the register
  allocator can commit to a per-thread budget instead of assuming the worst.
  m5-gemm's author calls it the single biggest practical win in the kernel; it is
  one line.
