...
bandwidth per core, double what contemporary NVIDIA and AMD parts move, so
kernels here reach for `simd_shuffle_xor` reductions without hesitation.
Production attention kernels do their online-softmax row reductions this way
...
One difference from CUDA: there is no Independent Thread Scheduling
